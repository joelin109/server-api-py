...
def crawl_article_body(crawl_url=None):
    ...
    # _url = 'https://www.nytimes.com/2017/05/02/fashion/met-gala-costume-institute.html?_r=0'
    _parse_rule = crawl_http_url_parse_rule(_url)
    if _parse_rule == '':
        return 'Please set parse rule first'
    else:
        print(_url + '  |  ' + _parse_rule)
        _crawl_body_text, _status = _crawler_article_body(_url, _parse_rule)
        return _crawl_body_text


def _crawler_article_body(url, parse_rule):
    ...
    if _soup_body_count == 0:
        ...
        _soup_body_html_text = _crawler_article_body_html_text(_soup_bodys[0])
        return _soup_body_html_text, _soup_body_count
    else:
        _soup_body_html_texts = ''
        for _soup_body in _soup_bodys:
            _soup_body_html_text = _crawler_article_body_html_text(_soup_body)
            _soup_body_html_texts += _soup_body_html_text + '<p></p><p></p>'

        return _soup_body_html_texts, _soup_body_count


def _crawler_article_body_html_text(soup):
    _soup_body = crawl_http_article_body_html(soup)
